...
export function targetMap(gameLength: number): Target[] {
  ...
  for (let i = 0; i < numMediumTargets; i++) {
    targetMap.push(
      generateTarget(getTargetType(), "medium", Math.random() * phase1Length)
    );
  }
  ...
  targetMap.sort((targetA, targetB) => targetA.spawnTime - targetB.spawnTime);

  // Log the generated target map (now sorted by spawn time)
  console.log("Generated Target Map (sorted by spawn time):", targetMap);
  return targetMap;
}
